...
const Crear = async (req, res) => {
  try {
    ...
    const registroFinal = Array.isArray(nuevoRegistro)
      ? nuevoRegistro[0]
      : nuevoRegistro;

    return ResponderExito(res, 'Se guardó el registro exitosamente.', registroFinal, 201);
  } catch (error) {
...
const Editar = async (req, res) => {
  try {
    const { Codigo } = req.params;
    const registroActualizado = await Servicio.Editar(Codigo, req.body);
    return ResponderExito(res, 'Se actualizó el registro exitosamente.', registroActualizado);
  } catch (error) {
